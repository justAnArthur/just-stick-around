...
export async function spotPlace(image: string, lat: number, lng: number) {
  ...
  const file = await saveBase64File(image, `spot-${spot.id}-user-${user.id}-${Date.now()}.png`)
...
async function saveBase64File(base64: string, fileName: string) {
  const base64Data = base64.replace(/^data:image\/\w+;base64,/, "")
  const buffer = Buffer.from(base64Data, 'base64')
  return saveBufferFile(buffer, fileName)
}
